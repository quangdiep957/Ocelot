...
public sealed class KubernetesServiceDiscoveryTests : Steps, IDisposable
{
    ...
    {
        // Arrange
        const int totalServices = 3, totalRequests = 50;
        const string namespaces = nameof(KubernetesServiceDiscoveryTests);
    ...
    private ConcurrentDictionary<int, int> _counters;
    //private RoundRobinCounter _roundRobinCounter;
    private int _k8sCounter, _k8sTotalFailed;
    ...
    private void GivenK8sProductServiceIsRunning(string url, string response)
    {
        ...
        _counters = new(2, 1); // single counter
    ...
    private void GivenMultipleK8sProductServicesAreRunning(List<string> urls, List<string> responses, int totalThreads)
    {
        ...
        _counters = new(totalThreads, urls.Count); // multiple counters
    ...
    private void GivenK8sProductServiceIsRunning(string url, string response, int handlerIndex)
    {
        ...
        serviceHandler.GivenThereIsAServiceRunningOn(url,
            async context =>
            {
                try
                {
                    _counters[handlerIndex]++;
                    var threadResponse = string.Concat(_counters[handlerIndex], ':', response);
    ...
    {
        _counters.Values.Sum().ShouldBe(expected);
    ...
    {
        Func<int, bool> shouldBeInRange = counter => bottom <= counter && counter <= top;
        _counters.Values.ShouldAllBe(c => shouldBeInRange(c));
        //_roundRobinCounter.Counters.Values.ShouldAllBe(c => shouldBeInRange(c));
    }
}
